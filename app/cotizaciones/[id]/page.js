  ...
  const generarPDF = async () => {
    try {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        // Imagen de dirección
        try {
          const direccionUrl = 'https://cxxifwpwarbrrodtzyqn.supabase.co/storage/v1/object/public/Logo/direccion.png'
          const direccionImg = await fetch(direccionUrl).then(r => r.blob()).then(blob => {
            return new Promise((resolve) => {
              const reader = new FileReader()
              reader.onloadend = () => resolve(reader.result)
              reader.readAsDataURL(blob)
          ...
          doc.addImage(direccionImg, 'PNG', 55, 270, 100, 15)
        } catch (error) {
    ...
    <>
      ...
      <div ref={printRef} style={{ display: 'none', width: '900px', padding: '40px', backgroundColor: 'white', fontFamily: 'Arial, sans-serif' }}>
        ...
        <div style={{ textAlign: 'center', marginTop: '30px' }}>
          <img 
            src="https://cxxifwpwarbrrodtzyqn.supabase.co/storage/v1/object/public/Logo/direccion.png"
