...
export default function VerCotizacion() {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      if (user.usuario === 'tintasytecnologia1') {
        setUserPhone('3102605693')
      } else if (user.usuario === 'tintasytecnologia2') {
  ...
  const generarPDF = async () => {
    try {
      ...
      // Agregar logo
      try {
        const logoUrl = 'https://cxxifwpwarbrrodtzyqn.supabase.co/storage/v1/object/public/Logo/logo%20circulo.svg'
        ...
        doc.addImage(logoImg, 'SVG', 15, 6, 30, 30)
      } catch (error) {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        if (userPhone) {
          ...
          doc.setFont('helvetica', 'bold')
          doc.setTextColor(...primaryColor)
          doc.text(`📞 Contacto: ${userPhone}`, 105, 272, { align: 'center' })
        }
        ...
        doc.text(`Tintas y Tecnología - Cotización #${cotizacion.numero}`, 105, 283, { align: 'center' })
    ...
    <>
      ...
      <div ref={printRef} style={{ display: 'none', width: '900px', padding: '40px', backgroundColor: 'white', fontFamily: 'Arial, sans-serif' }}>
        ...
        <div style={{ backgroundColor: '#f00000', padding: '30px', marginBottom: '30px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          <img 
            src="https://cxxifwpwarbrrodtzyqn.supabase.co/storage/v1/object/public/Logo/logo%20circulo.svg" 
            alt="Logo" 
        ...
        {userPhone && (
          <div style={{ textAlign: 'center', marginTop: '30px', padding: '20px', backgroundColor: '#f9fafb', borderRadius: '8px' }}>
            <p style={{ fontSize: '16px', fontWeight: 'bold', color: '#333', marginBottom: '8px' }}>
              Contacto del Vendedor
            </p>
            <p style={{ fontSize: '18px', fontWeight: 'bold', color: '#f00000' }}>
              📞 {userPhone}
            </p>
          </div>
        )}
